...
def run(model):
    ...
    # hidden_scale_up=True: without it PairformerStack builds c_hidden 128
    # whatever c_z is, so protenix2 (c_z 256) fails load_state_dict with a wall
    # of size mismatches. trunk_parity.py passes it for the same reason.
    net = PairformerStack(n_blocks=d['n_pairformer'], n_heads=n_single_heads,
                          c_z=d['c_z'], c_s=d['c_s'], hidden_scale_up=True)
    ...
    for j in range(d['n_msa']):
        ...
        # hidden_scale_up for the same reason PairformerStack needs it: at
        # c_z 256 the pair stack's hidden is 256 and the tri-attention 8 heads,
        # where the class defaults are 128/4 -- protenix2 fails load_state_dict
        # without it, which is what left this model's MSA module ungated.
        blk = MSABlock(c_m=c_m, c_z=d['c_z'], is_last_block=last,
                       hidden_scale_up=d['c_z'] > 128)
