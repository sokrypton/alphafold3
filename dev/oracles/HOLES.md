...
| `L3.denoise` | boltz2 | still open, and deliberately last: its inputs are the atom encoder and token transformer this session just gave adapters, and `DiffusionModule.forward` wants the whole `diffusion_conditioning` dict plus s_inputs at boltz's token_s width, not AF3's 449. |
